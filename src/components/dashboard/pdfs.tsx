        ...
        <AnimatePresence>
          {pdfs.map((pdf, index) => (
            <motion.div
              ...
              className="bg-[#1a0f24] rounded-lg border border-purple-900/20 overflow-hidden hover:border-purple-500/30 transition-all duration-200 group shadow-lg shadow-purple-900/5"
              onMouseEnter={() => setHoveredPdf(pdf.id)}
              onMouseLeave={() => setHoveredPdf(null)}
            >
              <div className="relative aspect-[4/3] bg-[#0e0525] flex items-center justify-center overflow-hidden">
                <motion.div
                  animate={{ 
                    rotate: [0, 5, 0, -5, 0],
                    scale: [1, 1.05, 1],
      ...
      <AnimatePresence>
        {pdfs.map((pdf, index) => (
          <motion.div
            ...
            exit={{ opacity: 0, x: 20 }}
            transition={{ duration: 0.3, delay: index * 0.05 }}
            className="p-4 bg-[#1a0f24] hover:bg-[#251539] rounded-lg transition-colors flex items-center justify-between group shadow-md shadow-purple-900/5 border border-purple-900/20 hover:border-purple-500/30"
          >
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-lg bg-[#0e0525] flex items-center justify-center">
                <motion.div
                  animate={{ 
                    rotate: [0, 5, 0, -5, 0],
                  }}
              ...
              <div>
                <h3
                  className="font-medium text-white truncate max-w-xs group-hover:text-purple-400 transition-colors"
                  title={pdf.name}
                >
                  {pdf.name}
                </h3>
                <div className="flex items-center text-xs text-white/50 mt-1">
                  ...
                  <span>{pdf.createdAt ? formatDate(pdf.createdAt) : "Recentemente enviado"}</span>
                </div>
              </div>
            </div>
