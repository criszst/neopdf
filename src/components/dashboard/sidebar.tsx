...
import {
  Settings, Star, Clock, Search, ChevronDown,
  LayoutGrid, Folder, BarChart3, Calendar,
  X,
} from "lucide-react"
...
const SideBar: React.FC<SideBarProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-50 w-64  bg-[#1a0f24] transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0`}
    >
      <div className="flex flex-col h-full">
        {/* Logo */}
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center">
            <span className="text-2xl font-bold bg-gradient-to-r from-purple-400 via-purple-500 to-purple-600 bg-clip-text text-transparent">
              Neo
            </span>
        ...
        <nav className="flex-1 px-4 space-y-1">
          {[
            ...
            <button
              key={item.label}
              className={`flex items-center space-x-3 w-full px-3 py-2 rounded-lg transition-all duration-200 ${item.active
        ...
        {/* User */}
        <div className="p-4 border-purple-900/20 md:lg:hidden">
